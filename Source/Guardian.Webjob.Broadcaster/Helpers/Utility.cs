...
namespace Guardian.Webjob.Broadcaster
{
    ...
    {
        private static string _smsPanicMessage = Common.Resources.Messages.SMSMessage;
        private static string _emailPanicMessage = Common.Resources.Messages.EmailMessage;
        private static string _facebookPanicMessage = Common.Resources.Messages.FacebookMessage;
        private static string _emailPanicSubject = Common.Resources.Messages.EmailSubject;
        private static string _facebookPanicSubject = Common.Resources.Messages.FacebookSubject;

        public static string GetSMSBody(string TinyURL, string UserName, string UserAddress, string UserPhoneNumber)
        {
            return _smsPanicMessage.Replace("{name}", UserName).Replace("{phone}", UserPhoneNumber)
                .Replace("{address}", UserAddress).Replace("{tinyuri}", TinyURL);
        }

        public static string GetEmailBody(string TinyURL, string UserName, string UserAddress, string UserPhoneNumber, DateTime time)
        {
            return _emailPanicMessage.Replace("{name}", UserName).Replace("{phone}", UserPhoneNumber)
                .Replace("{address}", UserAddress).Replace("{tinyuri}", TinyURL).Replace("{time}",
        ...
        {
            return _facebookPanicMessage.Replace("{name}", UserName).Replace("{phone}", UserPhoneNumber)
                .Replace("{address}", UserAddress).Replace("{tinyuri}", TinyURL).Replace("{time}",
                time.ToLongDateString() + " " + time.ToLongTimeString());
        }
        ...
        public static List<Tuple<int, string, string>> GetGrpIdGroupKeyAndShapePaths(Settings settings)
        {
            ...
            {
                if (DateTime.UtcNow.Subtract(LastUpdateDate).TotalMinutes >= settings.TimeToResetCacheInMinutes)
                {
                    isCacheResetRequired = true;
                }
            }
