...
var BingMapsAndroid = new function () {
    ...
    this.LoadMap = function (mapId) {
        ...
        var pin = new Microsoft.Maps.Pushpin(location, { icon: 'pin_gps.png', width: 30, height: 50, typeName: 'mypin' });
    ...
    this.ProcessData=function(latlongValues,isSOS)
    {
        try
        {
            ...
            $("#lbl").val(jsonResponse[0].Lat+","+jsonResponse[0].Long+","+isSOS);
            ...
            {
                var loc[i] = new Microsoft.Maps.Location(jsonResponse[i].Lat, jsonResponse[i].Long);
        
            }     
            //for start pin
            var pin[0] = new Microsoft.Maps.Pushpin(loc[0],{ icon: 'pin_gps.png', width: 30, height: 50, typeName: 'mypin' }); 
        
            // for last pin       
            var pin[jsonResponse.length-1] = new Microsoft.Maps.Pushpin(loc[jsonResponse.length-1],{ icon: 'pin_gps.png', width: 30, height: 50, typeName: 'mypin' });
            
            _map.entities.push(pin);
        
            // Create a shape
            var line = new Microsoft.Maps.Polyline(loc,{strokeColor:new Microsoft.Maps.Color(100, 100, 0, 100)});
